...
export const AIProvider = {
  ...
  buildFieldSpec(mod) {
    ...
    for (const f of mod.fields) {
      ...
      const repeatNote =
        f.repeat && f.repeat > 1
          ? ` (generate exactly ${f.repeat} unique items as an array)`
          : "";
      lines.push(
        `- "${f.key}" (${typeLabel}${maxNote}): ${f.label}${repeatNote}`,
      );
    }
    return lines.join("\n");
  },
  ...
  async fetchClaude(config, prompt, options) {
    ...
    let systemPrompt =
      "You are a structured data generator. You MUST respond with ONLY a valid JSON object. No introductory text, no explanations, no markdown formatting — just the raw JSON object.";
  ...
  async generateChart(
    ...
    const strategyInstructions =
      STRATEGY_BLOCKS[strategy] || STRATEGY_BLOCKS.balanced;
    ...
    if (settings.aiPlatform === "openai") {
      const schema = {
        ...
        properties: {
            ...
            items: {
              ...
              properties: {
                metricName: {
                  type: "string",
                  description:
                    "The name of the comparison metric. Max 30 chars, title-case.",
                },
                values: {
                  type: "object",
                  ...
                  description:
                    "A mapping from ASIN to the value for this metric. Use '✔' for checkmarks, or short text (max 250 chars).",
                },
              },
          ...
          shortTitles: {
            ...
            additionalProperties: false,
            description:
              "A mapping from ASIN to a concise product title (max 80 chars). Strip SEO filler and brand repetition.",
          },
        },
      ...
      options.response_format = {
        type: "json_schema",
        json_schema: { name: "chart_generation", strict: true, schema: schema },
      };
  ...
  async generateGenericModule(productData, settings, mod) {
    ...
    const heroProduct = Array.isArray(productData)
      ? productData[0]
      : productData;
  ...
  async identifyOpportunities(products, settings) {
            ...
            items: {
              ...
              properties: {
                groupName: {
                  type: "string",
                  description:
                    "Clear, commercial name/title for the group. Max 250 chars, straight and precise description of main product.",
                },
                asins: {
                  type: "array",
                  items: { type: "string" },
    ...
    throw new Error(
      "AI response did not contain a valid list of opportunities.",
    );
  },

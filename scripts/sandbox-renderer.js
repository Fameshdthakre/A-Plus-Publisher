  ...
  renderGenericModule(container, chart, onUpdateCell, mode) {
    ...
    const createEditableText = (
      ...
      if (!isTextarea) {
        textSpan.style.fontWeight =
          label.toLowerCase().includes("header") ||
      ...
      textSpan.addEventListener("click", () => {
        const currentVal =
          textSpan.style.fontStyle === "italic" ? "" : textSpan.textContent;
    ...
    if (hasRepeat) {
      ...
      gridDiv.style.gridTemplateColumns =
        mode === "desktop" ? `repeat(${Math.min(maxRepeat, 4)}, 1fr)` : "1fr";
